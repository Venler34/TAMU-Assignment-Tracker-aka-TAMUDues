...
1. Separated Java Spring Backend into multiple layers to avoid sending requests between APIs and reducing repeated code  
2. Java Spring API is RESTFUL implementing CRUD operations (stateless so no storing data about client on server  / no dependencies between API endpoints)  
3. JWT Tokens are implemented because they are compact, URL-safe easy to add authentication header and use them to find out what users are logged in + no need to store state on server side  
4. Built Docker Image for Java Microservice to allow for consistent behavior if ever deployed
